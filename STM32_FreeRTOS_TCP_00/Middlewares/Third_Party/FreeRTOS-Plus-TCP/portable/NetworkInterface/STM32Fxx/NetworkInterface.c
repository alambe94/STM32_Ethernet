...
BaseType_t xNetworkInterfaceInitialise(void)
{
    ...
    if (xEMACTaskHandle == NULL)
    {
        ...
        __HAL_ETH_DMA_ENABLE_IT(&heth, ETH_DMA_ALL_INTS);

        /* Initialize Tx Descriptors list: Chain Mode */
        HAL_ETH_DMATxDescListInit(&heth, DMATxDscrTab, Tx_Buff[0], ETH_TXBUFNB);

        /* Initialize Rx Descriptors list: Chain Mode  */
        HAL_ETH_DMARxDescListInit(&heth, DMARxDscrTab, Rx_Buff[0], ETH_RXBUFNB);

#if (ipconfigUSE_LLMNR != 0)
        {
            /* Program the LLMNR address at index 1. */
            prvMACAddressConfig(&heth, ETH_MAC_ADDRESS1, (uint8_t *)xLLMNR_MACAddress);
        }
#endif
...
void vMACBProbePhy(void)
{
    ...
    HAL_ETH_ReadPHYRegister(&heth, PHY_REG_03_PHYSID2, &ulLower);
    HAL_ETH_ReadPHYRegister(&heth, PHY_REG_02_PHYSID1, &ulUpper);
    ...
    HAL_ETH_ReadPHYRegister(&heth, PHY_REG_00_BMCR, &ulConfig);

    HAL_ETH_WritePHYRegister(&heth, PHY_REG_00_BMCR, ulConfig | BMCR_RESET);
    xRemTime = (TickType_t)pdMS_TO_TICKS(1000UL);
    vTaskSetTimeOutState(&xPhyTime);

    for (;;)
    {
        HAL_ETH_ReadPHYRegister(&heth, PHY_REG_00_BMCR, &ulValue);
    ...
    HAL_ETH_WritePHYRegister(&heth, PHY_REG_04_ADVERTISE, ulAdvertise);
    ...
    */
    ...
    HAL_ETH_ReadPHYRegister(&heth, PHY_REG_19_PHYCR, &ulPhyControl);
    ...
    HAL_ETH_WritePHYRegister(&heth, PHY_REG_19_PHYCR, ulPhyControl);
...
static void prvEthernetUpdateConfig(BaseType_t xForce)
{
    ...
    if ((xForce != pdFALSE) || ((ulPHYLinkStatus & BMSR_LINK_STATUS) != 0))
    {
        ...
        {
            /* Enable Auto-Negotiation. */
            HAL_ETH_WritePHYRegister(&heth, PHY_BCR, ulBCRvalue | BMCR_ANRESTART);
            HAL_ETH_WritePHYRegister(&heth, PHY_REG_04_ADVERTISE, ulACRValue);

            /* Wait until the auto-negotiation will be completed */
            do
            {
                ulTimeout++;
                HAL_ETH_ReadPHYRegister(&heth, PHY_REG_01_BMSR, &ulRegValue);
        ...
        else /* AutoNegotiation Disable */
        {
            ...
            usValue = (uint16_t)(heth.Init.DuplexMode >> 3) | (uint16_t)(heth.Init.Speed >> 1);
            HAL_ETH_WritePHYRegister(&heth, PHY_BCR, usValue);
        }
...
static void prvEMACHandlerTask(void *pvParameters)
{
    ...
    for (;;)
    {
        ...
        {
            HAL_ETH_ReadPHYRegister(&heth, PHY_REG_01_BMSR, &xStatus);
